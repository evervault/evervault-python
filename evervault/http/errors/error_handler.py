...
def raise_errors_on_failure(resp, body = None):
  ...
  if resp.status_code == 404:
    ...
    raise errors.UnexpectedError(message_for_unexpected_error_without_type(body))
...
def message_for_unexpected_error_without_type(error_details):
  if error_details != None: 
    ...
    return "An unexpected error occured. It occurred with the message: %s and http_code: '%s'. Please contact Evervault support" % (message, status_code)
  else:
