...
class Client(object):
    ...
    def encrypt_data(self, fetch, data):
        ...
        shared_key = self.__derive_shared_key()
        ...
        if type(data) == dict:
            ...
            return self.__encrypt_string(data, shared_key)

    def __encrypt_object(self, data):
        if self.__encryptable_data(data):
            return self.__encrypt_string(data, None)
        elif type(data) == dict:
            encrypted_data = {}
    ...
    def __encrypt_string(self, data, shared_key):
        iv = token_bytes(12)
        aesgcm = AESGCM(shared_key)
        encrypted_buffer = aesgcm.encrypt(iv, bytes(data, 'utf8'), None)
