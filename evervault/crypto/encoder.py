...
def encode_p256_public_key(public_key):
    ...
    curve.setComponentByName(
        "seed", univ.BitString("'C49D360886E704936A6678E1139D26B7819F7E90'H")
    )

    field_id = FieldID()
    field_id.setComponentByName(
        "fieldType", univ.ObjectIdentifier((1, 2, 840, 10045, 1, 1))
    )
    field_id.setComponentByName(
        "parameters",
        univ.Integer(
    ...
    algorithm_identifier.setComponentByName(
        "algorithm", univ.ObjectIdentifier((1, 2, 840, 10045, 2, 1))
    )
    ...
    subject_public_key_info.setComponentByName(
        "subjectPublicKey", univ.BitString("'{}'H".format(public_key))
    )
