...
class PublicKey:
  ...
  def build_encoder(self):
    ...
    curve.setComponentByName(
        "a",
        univ.OctetString(
            hexValue=self.curve_info.a
        ),
    )
    curve.setComponentByName(
        "b",
        univ.OctetString(
            hexValue=self.curve_info.b
        ),
    )
    curve.setComponentByName(
    ...
    field_id.setComponentByName(
        "fieldType", univ.ObjectIdentifier(PRIME_FIELD)
    )
    field_id.setComponentByName(
        "parameters",
    ...
    ec_parameters.setComponentByName(
        "base",
        univ.OctetString(
            hexValue=self.curve_info.generator
        )
    )
    ec_parameters.setComponentByName("order", univ.Integer(int(self.curve_info.n, 16)))
